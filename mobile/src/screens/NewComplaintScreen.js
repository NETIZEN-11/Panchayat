  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={[styles.categoryBtn, { borderColor: selectedColor }]} onPress={() => setShowCategoryModal(true)}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.categoryBtnDesc} numberOfLines={1}>{selectedCategory?.desc}</Text>
          </View>
            ...
            <ScrollView>
              {CATEGORIES.map(cat => {
                ...
                return (
                  <TouchableOpacity
                    key={cat.value}
                    style={[styles.catOption, category === cat.value && { backgroundColor: catColor + '15' }]}
                    onPress={() => { setCategory(cat.value); setShowCategoryModal(false); }}
                  >
                    <View style={[styles.catDot, { backgroundColor: catColor }]} />
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.catOptionText, category === cat.value && { color: catColor }]}>{cat.label}</Text>
                      <Text style={styles.catOptionDesc} numberOfLines={1}>{cat.desc}</Text>
                    </View>
      ...
      <View style={styles.section}>
        ...
        <TextInput style={styles.input} placeholder="Short title (e.g. Road broken near school)" value={title} onChangeText={setTitle} />
      </View>
      ...
      {/* Images */}
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>PHOTO EVIDENCE (MAX 5)</Text>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.villageDisplay, { borderColor: '#27ae60' }]}>
          <Text style={[styles.villageText, { color: '#27ae60' }]}>{user?.village || 'Loaded from your profile'}</Text>
          <Text style={styles.villageNote}>Auto-filled from your account</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  section: {
    backgroundColor: '#fff', marginHorizontal: 15, marginTop: 12,
    borderRadius: 12, padding: 15, elevation: 2,
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3,
  },
  sectionTitle: { fontSize: 11, fontWeight: '800', color: '#95a5a6', marginBottom: 10, letterSpacing: 1.5 },
  input: {
    backgroundColor: '#f8f9fa', borderRadius: 8, padding: 13,
    borderWidth: 1, borderColor: '#dfe6e9', fontSize: 14, color: '#2c3e50',
  },
  textArea: { height: 100, textAlignVertical: 'top', marginTop: 4 },

  categoryBtn: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8f9fa',
    borderRadius: 10, borderWidth: 2, padding: 13,
  },
  categoryDot: { width: 14, height: 14, borderRadius: 7, marginRight: 12 },
  categoryBtnText: { fontSize: 16, fontWeight: '700' },
  categoryBtnDesc: { fontSize: 11, color: '#7f8c8d', marginTop: 2 },
  ...
  catOptionText: { fontSize: 15, color: '#2c3e50', fontWeight: '600' },
  catOptionDesc: { fontSize: 11, color: '#95a5a6', marginTop: 2 },
  ...
  gpsBtn: {
    backgroundColor: '#3498db', borderRadius: 8, padding: 13,
  ...
  imgBtn: {
    flex: 1, backgroundColor: '#f0f4f8', borderRadius: 10, padding: 14,
    alignItems: 'center', borderWidth: 1, borderColor: '#dfe6e9', borderStyle: 'dashed',
  },
  imgBtnText: { fontSize: 14, color: '#636e72', fontWeight: '600' },
  ...
  villageDisplay: { backgroundColor: '#f0f9f0', borderRadius: 8, borderWidth: 1, padding: 12 },
  villageText: { fontSize: 16, fontWeight: '700' },
  villageNote: { fontSize: 11, color: '#7f8c8d', marginTop: 2 },

  submitBtn: {
    backgroundColor: '#27ae60', margin: 15, borderRadius: 12,
    padding: 18, alignItems: 'center', elevation: 4,
    shadowColor: '#27ae60', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 6,
  },
