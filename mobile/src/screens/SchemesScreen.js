...
const STATIC_SCHEMES = [
  { _id: 'pmkisan', name: 'PM-KISAN', category: 'Agriculture', description: 'PM Kisan Samman Nidhi provides Rs.6,000/year to small farmers in 3 instalments.', eligibility: 'Small and marginal farmers with less than 2 hectares land', googleUrl: 'https://pmkisan.gov.in', officialUrl: 'https://pmkisan.gov.in', tag: 'AGR' },
  { _id: 'ayushman', name: 'Ayushman Bharat (PM-JAY)', category: 'Health', description: 'Health insurance cover of Rs.5 lakh per family per year for hospitalization.', eligibility: 'Poor and vulnerable families as per SECC 2011 data', googleUrl: 'https://www.google.com/search?q=Ayushman+Bharat+PM-JAY+scheme+apply', officialUrl: 'https://setu.pmjay.gov.in', tag: 'HLT' },
  { _id: 'pmay', name: 'PM Awas Yojana (Gramin)', category: 'Housing', description: 'Financial assistance to build pucca houses for BPL households in rural areas.', eligibility: 'BPL families without pucca houses in rural areas', googleUrl: 'https://www.google.com/search?q=PMAY+Gramin+rural+housing+scheme+apply', officialUrl: 'https://pmayg.nic.in', tag: 'HSG' },
  { _id: 'mnrega', name: 'MGNREGA', category: 'Employment', description: 'Guarantees 100 days of wage employment per year to rural households.', eligibility: 'Adult members of rural households willing to do unskilled manual work', googleUrl: 'https://www.google.com/search?q=MGNREGA+job+card+apply+rural', officialUrl: 'https://mnregaweb4.nic.in', tag: 'EMP' },
  { _id: 'pmkvy', name: 'PM Kaushal Vikas Yojana', category: 'Skill', description: 'Skill training programme to enable Indian youth to take up industry-relevant training.', eligibility: 'School/college dropouts or unemployed youth (15-45 years)', googleUrl: 'https://www.google.com/search?q=PM+Kaushal+Vikas+Yojana+training+centers+apply', officialUrl: 'https://www.pmkvyofficial.org', tag: 'SKL' },
  { _id: 'jandhan', name: 'Jan Dhan Yojana', category: 'Finance', description: 'Zero balance bank accounts with RuPay debit card and accident insurance of Rs.2 lakh.', eligibility: 'Any Indian citizen above 10 years without a bank account', googleUrl: 'https://www.google.com/search?q=Jan+Dhan+Yojana+account+open+bank', officialUrl: 'https://pmjdy.gov.in', tag: 'FIN' },
  { _id: 'ujjwala', name: 'PM Ujjwala Yojana', category: 'Social', description: 'Provides free LPG connection to women from below-poverty-line households.', eligibility: 'Adult women from BPL households without existing LPG connection', googleUrl: 'https://www.google.com/search?q=PM+Ujjwala+Yojana+free+LPG+apply', officialUrl: 'https://pmuy.gov.in', tag: 'SOC' },
  { _id: 'swachhbharat', name: 'Swachh Bharat Mission', category: 'Sanitation', description: 'Free toilet construction for rural households and promotion of sanitation awareness.', eligibility: 'Rural households without toilets (BPL and APL listed)', googleUrl: 'https://www.google.com/search?q=Swachh+Bharat+Mission+rural+toilet+apply', officialUrl: 'https://swachhbharatmission.gov.in', tag: 'SAN' },
];
  ...
  const renderScheme = ({ item }) => {
    ...
    return (
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        ...
        <View style={[styles.eligibilityBox, { backgroundColor: colors.inputBg }]}>
          <Text style={styles.eligibilityLabel}>Eligibility:</Text>
          <Text style={[styles.eligibilityText, { color: colors.text }]}>{item.eligibility}</Text>
        </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.filterBar, { backgroundColor: colors.surface }]}>
        <TouchableOpacity style={[styles.dropdown, { backgroundColor: colors.inputBg, borderColor: colors.border }]} onPress={() => setShowDropdown(true)}>
          <Text style={[styles.dropdownText, { color: colors.text }]}>{selectedCat?.label || 'All Categories'}</Text>
          <Text style={[styles.dropdownArrow, { color: colors.textSecondary }]}>v</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.toggleBtn, { backgroundColor: colors.inputBg, borderColor: colors.border }, showStatic && styles.toggleBtnActive]}
          onPress={() => setShowStatic(!showStatic)}
        >
          <Text style={[styles.toggleBtnText, { color: colors.textSecondary }, showStatic && { color: '#fff' }]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            <FlatList
              ...
              renderItem={({ item: cat }) => (
                <TouchableOpacity
                  style={[styles.catOption, { borderBottomColor: colors.border }, category === cat.value && styles.catOptionSelected]}
                  onPress={() => { setCategory(cat.value); setShowDropdown(false); }}
                >
                  <Text style={[styles.catOptionText, { color: colors.text }, category === cat.value && styles.catOptionTextSelected]}>
                    {cat.label}
                  </Text>
                  {cat.url && (
      ...
      {loading && allSchemes.length === 0 ? (
        ...
        <View style={styles.centered}>
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No schemes found</Text>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  filterBar: { padding: 12, flexDirection: 'row', gap: 10, elevation: 2 },
  dropdown: { flex: 1, flexDirection: 'row', alignItems: 'center', borderRadius: 10, borderWidth: 1, padding: 10 },
  dropdownText: { flex: 1, fontSize: 14, fontWeight: '600' },
  dropdownArrow: { fontSize: 12 },
  toggleBtn: { borderRadius: 10, borderWidth: 1, padding: 10, justifyContent: 'center' },
  ...
  catOption: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1 },
  catOptionSelected: { backgroundColor: '#e8f4fd' },
  catOptionText: { flex: 1, fontSize: 15, fontWeight: '600' },
  ...
  card: { borderRadius: 16, marginBottom: 15, overflow: 'hidden', elevation: 3 },
  ...
  description: { fontSize: 14, padding: 14, lineHeight: 20 },
  eligibilityBox: { marginHorizontal: 14, borderRadius: 8, padding: 10, marginBottom: 12 },
  eligibilityLabel: { fontSize: 12, fontWeight: 'bold', color: '#27ae60', marginBottom: 3 },
  eligibilityText: { fontSize: 13 },
  ...
  emptyText: { fontSize: 18, fontWeight: '600', marginBottom: 20 },
